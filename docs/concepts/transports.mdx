...
模型上下文协议（MCP）中的传输为客户端和服务器之间的通信提供了基础。传输处理消息如何发送和接收的底层机制。
...
SSE 传输启用服务器到客户端的流式传输，客户端到服务器的通信则使用 HTTP POST 请求。
...
MCP 使实现特定需求的自定义传输变得容易。任何传输实现只需要符合传输接口：
...
<Tabs>
  ...
  <Tab title="Python">
    注意，虽然 MCP 服务器通常使用 asyncio 实现，但我们建议
    使用 `anyio` 实现底层接口如传输，以获得更广泛的兼容性。
    ```python
    @contextmanager
